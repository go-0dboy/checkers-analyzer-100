 ...
 * Точная теория для малофигурных окончаний: чисто дамочные,
 * шашки против одинокой дамки, решающий материальный перевес.
 * ============================================================ */
...
export function materialVerdict(m: Material): TbVerdict | null {
  ...
  if (wM === 0 && bM === 0) {
    if (wK >= 3 && bK === 1) return { result: 1, confidence: 'theory', note: 'большинство 3+ дамок против одной — выигрыш' };
    if (bK >= 3 && wK === 1) return { result: -1, confidence: 'theory', note: 'большинство 3+ дамок против одной — выигрыш' };
    if ((wK <= 2 && bK <= 2) || wK === bK) return { result: 0, confidence: 'theory', note: 'равные дамки не ловятся — теоретическая ничья' };
    return null;
  }
