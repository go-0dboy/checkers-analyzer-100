 ...
 * Нумерация полей 1–50, белый угол a10 слева снизу.
 * Реализовано: обязательное взятие (вперёд и назад), правило
 * большинства, летающие дамки, превращение только при остановке
 * на последней горизонтали, блокировка побитых шашек в серии.
 * ============================================================ */
...
/** Серии взятий простой шашкой. Побитые шашки остаются и блокируют траекторию. */
...
/** Все легальные ходы позиции (с учётом правила большинства). */
...
export function applyMove(pos: Pos, m: Move): Pos {
  ...
  // превращение — только при завершении хода на последней горизонтали
  if (v === WHITE && m.captures.length === 0 && ROW_OF[m.to] === 0) v = 2;
  else if (v === BLACK && m.captures.length === 0 && ROW_OF[m.to] === 9) v = -2;
  else if (v === WHITE && ROW_OF[m.to] === 0 && m.captures.length > 0) {
    /* при взятии простая, прошедшая последнюю горизонталь и остановившаяся
       на ней, становится дамкой — ряд 0/9 является конечной клеткой */
    v = 2;
  } else if (v === BLACK && ROW_OF[m.to] === 9 && m.captures.length > 0) {
    v = -2;
  }
  b[m.to] = v;
...
/* ---------- стартовая позиция и FEN (формат Liens / PDN) ---------- */
...
export function boardToFen(b: Int8Array, side: Side = WHITE): string {
  ...
  flushW(); flushB();
  const W = w.length ? w.join(',') : '';
  const B = bl.length ? bl.join(',') : '';
  return `${side === WHITE ? 'W' : 'B'}:W${W}:B${B}`;
}
...
/** Темпы (разность темпов) — для показа на панели анализа. */
