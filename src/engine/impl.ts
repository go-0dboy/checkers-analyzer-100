/* ============================================================
 * Имплементация движка: работает в Web Worker (impl.ts собирается
 * Vite как отдельный модуль воркера) либо в главном потоке
 * через runDirect — протокол сообщений одинаковый.
 *
...
async function runEngine(msg: GoMsg, post: (m: unknown) => void): Promise<void> {
  ...
  /* база фигур: точный вердикт для малофигурных окончаний */
  ...
  if (msg.startFen === START_FEN && msg.history.length <= 26) {
    ...
    if (hit && findMove(legal, hit.from, hit.to)) {
      ...
      post({
        ...
        mate: false, book: bookName, tb,
      } satisfies EngineResult & { t: string });
      return;
    }
  }
