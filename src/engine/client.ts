/* ============================================================
 * Клиент движка: запускает имплементацию в Web Worker и общается
 * по протоколу go/info/done/loadNet. Если воркер недоступен —
 * прозрачный фолбэк в главный поток через динамический импорт.
 * ============================================================ */
...
class EngineClient {
  ...
  private seq = 0;
  private netJson: string | null = null;
  private netSentDirect = false;
  ...
  /** Загрузить веса нейросети в движок (или null — выключить). */
  setNet(json: string | null): void {
    this.netJson = json;
    this.netSentDirect = false;
    if (this.worker) this.worker.postMessage({ t: 'loadNet', net: json });
    else if (this.directMod) { this.directMod.loadNetDirect(json); this.netSentDirect = true; }
  }

  private ensureNetDirect(): void {
    if (this.direct && !this.netSentDirect && this.directMod) {
      this.directMod.loadNetDirect(this.netJson);
      this.netSentDirect = true;
    }
  }
