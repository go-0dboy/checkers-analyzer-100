...
/* v3: прежние сохранения (сделанные до исправлений доски) несовместимы */
...
export function useGame(opts: GameOptions) {
  ...
  /* ----- авто-взятие -----
     Обязательное взятие выполняется само (варианты уже отфильтрованы
     ядром по правилу большинства; из равных берём ход движка, если он
     готов, иначе первый). Только на «живом» конце партии, чтобы не
     переписывать историю при просмотре. */
  ...
  const clickSquare = useCallback((n: number) => {
    ...
    if (movableFroms.has(n) && pos.b[n] * pos.side > 0) {
      /* авто-ход: единственный пункт назначения — ходим сразу */
      if (opts.autoSingle) {
