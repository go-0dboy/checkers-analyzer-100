    ...
    <div className="flex flex-col gap-4">
      <section className="panel p-4">
        <header className="mb-3 flex items-center gap-2">
          <IconChip size={16} className="text-acc" />
          <h2 className="font-display text-[11px] font-bold tracking-[.22em] text-mut">
          ...
          {ENGINES.map((e) => (
            <article
              ...
              className={`rounded-md border p-3 transition-colors ${
                e.pick
                  ? 'border-acc/45 bg-acc/7 hover:bg-acc/11'
            ...
            >
              <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
                ...
                {e.pick && (
                  <span className="rounded bg-acc/20 px-1.5 py-0.5 text-[10px] font-bold tracking-wide text-acc2">
                    РЕКОМЕНДОВАН
                  </span>
                )}
                <span className="ml-auto font-mono text-[10px] text-dim">{e.lang}</span>
              </div>
              <div className="mt-1 flex flex-wrap gap-x-3 gap-y-0.5 text-[11px] text-mut">
                <span>{e.author}</span>
                <span className="text-dim">·</span>
                <span className="text-[#8ed0ae]">{e.license}</span>
                <span className="text-dim">·</span>
                <span>{e.strength}</span>
              </div>
              <p className="mt-1.5 text-xs leading-relaxed text-mut">{e.note}</p>
            </article>
          ))}
        </div>
      </section>
      ...
      <section className="panel p-4">
        <h2 className="mb-3 font-display text-[11px] font-bold tracking-[.22em] text-mut">
          ...
          {['UI · React (этот MVP)', 'Worker / нативный мост', 'Scan WASM / бинарь', 'Liens 2–7 · API'].map((box, i, arr) => (
            <div key={box} className="flex items-center gap-2">
              <div className={`rounded border px-2.5 py-1.5 font-mono text-[10px] sm:text-[11px] ${
                i === 0 ? 'border-acc/50 bg-acc/10 text-acc2' :
              ...
              {i < arr.length - 1 && (
                <svg width="18" height="10" viewBox="0 0 18 10" className="text-dim">
        ...
        <ol className="mt-3 flex flex-col gap-2.5">
          {STEPS.map((s) => (
            <li key={s.title} className="rounded-md border border-white/8 bg-white/[.025] p-3 transition-colors hover:bg-white/[.05]">
              <h3 className="text-xs font-bold text-ink">{s.title}</h3>
              <p className="mt-1 text-xs leading-relaxed text-mut">{s.text}</p>
            </li>
          ))}
        </ol>
      </section>
      ...
      <section className="panel p-4">
        <h2 className="mb-2 font-display text-[11px] font-bold tracking-[.22em] text-mut">
          ПАМЯТКА ПО ПРАВИЛАМ ФМЖД В ЯДРЕ
        </h2>
        <ul className="grid gap-1.5 text-xs leading-relaxed text-mut sm:grid-cols-2">
